...
// We need a reasonable upper bound to avoid enormous stack allocations in the
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
  ...
  case 2: // Fixed Buffer Allocator
  {
    ...
    size_t buf_size = ((Data[offset] << 8) | Data[offset + 1]) %
                          (active_max_alloc * active_max_pointers + 1) +
                      1;
    offset += 2;
    backing_buffer = malloc(buf_size);
    if (make_fixed_buf_allocator(&a, backing_buffer, buf_size) != ALLO_OK) {
      free(backing_buffer);
      return 0;
    }
    is_complex = 1;
  ...
  case 3: // Arena Allocator
  {
    ...
    offset += 2;
    if (make_c_allocator(&child) != ALLO_OK)
      return 0;
    if (make_arena_allocator(&a, &child, block_size) != ALLO_OK)
      return 0;
    is_complex = 2;
    break;
  }
  case 4: // Pool Allocator
  {
    ...
    size_t pool_block_size =
        ((Data[offset] << 8) | Data[offset + 1]) % 4096 + 1;
    offset += 2;
    ...
    if (make_c_allocator(&child) != ALLO_OK)
      return 0;
    if (make_pool_allocator(&a, &child, NULL, pool_block_size, total_blocks) !=
        ALLO_OK)
      return 0;
    is_complex = 3;
    break;
  ...
  case 5: // Buddy Allocator
  {
    ...
    if (make_c_allocator(&child) != ALLO_OK)
      return 0;
    if (make_buddy_allocator(&a, &child, NULL, buddy_size) != ALLO_OK)
      return 0;
    is_complex = 4;
    break;
  }
  ...
  while (offset < Size) {
    ...
    if (op == OP_ALLOC) {
      if (offset + 2 > Size)
        break;
      size_t alloc_size =
          ((Data[offset] << 8) | Data[offset + 1]) % active_max_alloc;
      offset += 2;
      ...
      tracked[index].magic =
          op_byte; // Use op_byte as unique magic for this allocation
      ...
      void *new_ptr =
          allo_realloc(&a, tracked[index].ptr, tracked[index].size, new_size);
