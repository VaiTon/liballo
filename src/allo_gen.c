...
static const size_t pool_sizes[POOL_COUNT] = {16, 32, 64, 128, 256, 512, 1024, 2048};
static const size_t pool_blocks[POOL_COUNT] = {4096, 2048, 1024, 512, 256, 128, 64, 32};
...
static void *gen_realloc_fn(allo_t *self, void *ptr, size_t old_size, size_t new_size) {
  if (!ptr) return gen_alloc_fn(self, new_size);
  if (new_size == 0) {
...
static allo_contains_t gen_contains_fn(allo_t *self, void *ptr) {
  ...
  for (int i = 0; i < POOL_COUNT; i++) {
    if (allo_contains(&s->pools[i], ptr) == ALLO_CONTAINS_YES) return ALLO_CONTAINS_YES;
  }
  if (allo_contains(&s->buddy, ptr) == ALLO_CONTAINS_YES) return ALLO_CONTAINS_YES;
  if (allo_contains(&s->page, ptr) == ALLO_CONTAINS_YES) return ALLO_CONTAINS_YES;

  return ALLO_CONTAINS_NO;
}

allo_error_t make_gen_allocator(allo_t *out) {
  if (!out) return ALLO_ERR_INVAL;

  // Use page allocator to bootstrap the internal state
  allo_t page;
  allo_error_t err = make_page_allocator(&page);
  if (err != ALLO_OK) return err;
  ...
  for (int i = 0; i < POOL_COUNT; i++) {
    err = make_pool_allocator(&s->pools[i], &s->buddy, NULL, pool_sizes[i], pool_blocks[i]);
    if (err != ALLO_OK) {
        // Cleanup already created pools
        for (int j = 0; j < i; j++) allo_destroy(&s->pools[j]);
        allo_destroy(&s->buddy);
        allo_free(&s->page, s, sizeof(gen_state_t));
        allo_destroy(&s->page);
        return err;
